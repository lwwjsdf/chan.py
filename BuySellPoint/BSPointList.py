...
class CBSPointList(Generic[LINE_TYPE, LINE_LIST_TYPE]):
    def __init__(self, bs_point_config: CBSPointConfig):
        self.bsp_store_dict: Dict[
            BSP_TYPE, Tuple[List[CBS_Point[LINE_TYPE]], List[CBS_Point[LINE_TYPE]]]
        ] = {}
    ...
    def cal_seg_bs1point(
        self, seg_list: CSegListComm[LINE_TYPE], bi_list: LINE_LIST_TYPE
    ):
    ...
    def cal_single_bs1point(self, seg: CSeg[LINE_TYPE], bi_list: LINE_LIST_TYPE):
        ...
        zs_cnt = (
            ...
            if BSP_CONF.bsp1_only_multibi_zs
            else len(seg.zs_lst)
        )
    ...
    def treat_bsp1(
        self, seg: CSeg[LINE_TYPE], BSP_CONF: CPointConfig, is_target_bsp: bool
    ):
    ...
    def cal_seg_bs2point(
        self, seg_list: CSegListComm[LINE_TYPE], bi_list: LINE_LIST_TYPE
    ):
        for seg in seg_list[self.last_sure_seg_idx :]:
            config = self.config.GetBSConfig(seg.is_down())
            if (
                BSP_TYPE.T2 not in config.target_types
                and BSP_TYPE.T2S not in config.target_types
            ):
                continue
            if not self.seg_need_cal(seg):
                continue
    ...
    def treat_bsp2(
        self, seg: CSeg, seg_list: CSegListComm[LINE_TYPE], bi_list: LINE_LIST_TYPE
    ):
        if len(seg_list) > 1:
            BSP_CONF = self.config.GetBSConfig(seg.is_down())
            bsp1_bi = seg.end_bi
            ...
            bsp2_bi = bi_list[1]
            break_bi = bi_list[0]
        if BSP_CONF.bsp2_follow_1 and (
            not bsp1_bi or bsp1_bi.idx not in self.bsp1_dict
        ):
            return
    ...
    def treat_bsp2s(
        ...
        while bsp2_bi.idx + bias < len(bi_list):  # 计算类二
            ...
            if bias == 2:
                if not has_overlap(
                    bsp2_bi._low(), bsp2_bi._high(), bsp2s_bi._low(), bsp2s_bi._high()
                ):
                    break
            ...
            if bsp2s_break_bsp1(bsp2s_bi, break_bi):
                break
            retrace_rate = (
                abs(bsp2s_bi.get_end_val() - break_bi.get_end_val()) / break_bi.amp()
            )
    ...
    def cal_seg_bs3point(
        ...
        for seg in seg_list[self.last_sure_seg_idx :]:
            ...
            config = self.config.GetBSConfig(seg.is_down())
            if (
                BSP_TYPE.T3A not in config.target_types
                and BSP_TYPE.T3B not in config.target_types
            ):
                continue
            if len(seg_list) > 1:
                bsp1_bi = seg.end_bi
                ...
                next_seg = (
                    seg.next
                )  # 可能为None, 所以并不一定可以保证next_seg_idx == next_seg.idx
            else:
                next_seg = seg
                ...
                bsp1_bi_idx = -1
                BSP_CONF = self.config.GetBSConfig(seg.is_up())
            if BSP_CONF.bsp3_follow_1 and (
                not bsp1_bi or bsp1_bi.idx not in self.bsp_store_flat_dict
            ):
                continue
            if next_seg:
    ...
    def treat_bsp3_before(
        ...
        if not bsp1_bi:
            return
        if BSP_CONF.strict_bsp3 and (
            cmp_zs.bi_out is None or cmp_zs.bi_out.idx != bsp1_bi.idx
        ):
            return
