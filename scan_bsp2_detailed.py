...
def scan_stock_worker(code: str) -> Optional[Dict]:
    """工作进程：扫描单只股票，返回详细分析结果"""
    try:
        # 在每个进程中创建新的配置
        config = CChanConfig(
            {
        ...
        print(f"  {code}: 找到 {len(bsp2_list)} 个二类买点")
        ...
        print(f"  {code}: 买点索引 {bsp_klu_idx}, 总K线数 {len(all_klus)}")
        ...
        for idx in range(bsp_klu_idx, len(all_klus)):
            ...
            if klu.lst and len(klu.lst) > 0:
                ...
                close_price = (
                    last_unit.close
                    if hasattr(last_unit, "close")
                    else last_unit.get_end_val()
                )
                high_price = klu.high if hasattr(klu, "high") else last_unit.high
                low_price = klu.low if hasattr(klu, "low") else last_unit.low
        ...
        print(f"  {code}: 买点后价格数据 {len(prices_after_bsp)} 条")
        ...
        # 计算涨幅
        gain_pct = (last_price - bsp_price) / bsp_price * 100 if bsp_price > 0 else 0
        max_gain_pct = (max_price - bsp_price) / bsp_price * 100 if bsp_price > 0 else 0
...
def main():
    parser = argparse.ArgumentParser(description="全市场二类买点详细分析（并行优化版）")
    parser.add_argument(
        "--output",
    ...
    print(f"使用 {args.workers} 个进程并行扫描...\n")

    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        future_to_code = {
            executor.submit(scan_stock_worker, code): code for code in stocks
        }
        ...
        for future in as_completed(future_to_code):
            ...
            try:
                ...
                if result:
                    ...
                    print(
                        f"[{completed}/{len(stocks)}] {code} {status} "
                        f"{result['bsp_date']} {result['bsp_type']} "
                        f"gain={result['gain_pct']:+.1f}% "
                        f"max={result['max_gain_pct']:+.1f}% "
                        f"days={result['days_since_bsp']}"
                    )
                else:
    ...
    if results:
        ...
        print("-" * 80)
        print(
            f"{'排名':<4} {'代码':<8} {'日期':<12} {'类型':<8} {'买点价':<8} {'最新价':<8} "
            f"{'涨幅':<8} {'最大涨幅':<8} {'回撤':<8} {'天数':<4} {'状态':<6}"
        )
        print("-" * 80)
        ...
        for i, r in enumerate(results[:50], 1):
            ...
            print(
                f"{i:<4} {r['code']:<8} {r['bsp_date']:<12} {r['bsp_type']:<8} "
                f"{r['bsp_price']:<8.2f} {r['last_price']:<8.2f} "
                f"{r['gain_pct']:<+7.1f}% {r['max_gain_pct']:<+7.1f}% "
                f"{r['pullback_pct']:<7.1f}% {r['days_since_bsp']:<4} {status:<6}"
            )
